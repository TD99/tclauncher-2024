...
namespace TCLauncher.MVVM.View
{
    public partial class HealthSheet
    {
        ...
        private void Backups_Click(object sender, RoutedEventArgs e) => _ =
            AppServices.Overlays.ShowSheetAsync("Manage backups", new BackupManagerSheet(_instance), false);

        private async void Support_Click(object sender, RoutedEventArgs e)
        {
            if (!await AppServices.Overlays.ConfirmAsync("Export support bundle",
                    BundlePreview.Text + "\n\nNothing is sent automatically.", "Choose destination", "Cancel")) return;
            var picker = new SaveFileDialog
                { FileName = "TCLauncher-support.zip", DefaultExt = ".zip", Filter = "ZIP archive (*.zip)|*.zip" };
