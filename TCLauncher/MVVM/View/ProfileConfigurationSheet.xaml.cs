...
namespace TCLauncher.MVVM.View
{
    public partial class ProfileConfigurationSheet
    {
        ...
        {
            _instance = instance; _changed = changed; InitializeComponent();
        ...
        private void Save_Click(object sender, RoutedEventArgs e)
        {
            ...
            { Error.Text = "Enter a name and a valid memory range."; return; }
            _instance.DisplayName = DisplayName.Text.Trim(); _instance.MinimumRamMb = minimum; _instance.MaximumRamMb = maximum;
            _instance.JVMArguments = JvmArguments.Text.Split(new[] { "\r\n", "\n" }, StringSplitOptions.RemoveEmptyEntries).Select(value => value.Trim()).Where(value => value.Length > 0).ToArray();
            ...
            _changed?.Invoke(_instance); AppServices.Overlays.Close(true); AppServices.Overlays.ShowToast("Profile saved", _instance.DisplayName);
        }
    }
}
