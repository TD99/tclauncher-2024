...
namespace TCLauncher.Core.Services
{
    ...
    public sealed class OperationCoordinator : IOperationCoordinator
    {
            ...
            Func<IProgress<OperationProgress>, CancellationToken, Task<OperationResult<T>>> operation)
        {
            ...
            {
                return OperationResult<T>.Failure(LauncherErrorCode.Cancelled, "Operation cancelled.", exception,
                    current.Id);
            }
            catch (Exception exception)
            {
                return OperationResult<T>.Failure(LauncherErrorCode.Unexpected, exception.Message, exception,
                    current.Id);
            }
            finally
            {
