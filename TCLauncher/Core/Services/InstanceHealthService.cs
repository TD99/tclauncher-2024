...
namespace TCLauncher.Core.Services
{
    ...
    public sealed class InstanceHealthService : IInstanceHealthService
    {
        ...
        public InstanceHealthReport Inspect(InstalledInstance instance)
        {
            ...
            {
                report.Checks.Add(Check("Installation", "The profile folder is missing.", HealthSeverity.Error, "repair"));
            ...
            report.Checks.Add(Check("Loader", loader.Type + (string.IsNullOrWhiteSpace(loader.Version) ? string.Empty : " " + loader.Version), HealthSeverity.Information));
            if (loader.Type == LoaderType.NeoForge && CompareMinecraftVersions(instance.McVersion, "1.20.2") < 0)
                report.Checks.Add(Check("NeoForge compatibility", "NeoForge profiles require Minecraft 1.20.2 or newer.", HealthSeverity.Error, "edit"));

            if (Directory.Exists(instance.InstallationDir))
                report.StorageBytes = Directory.GetFiles(instance.InstallationDir, "*", SearchOption.AllDirectories).Sum(path => new FileInfo(path).Length);
            report.Checks.Add(Check("Storage", FormatBytes(report.StorageBytes), HealthSeverity.Information, "open-folder"));
            ...
            report.Checks.Add(report.LatestBackup == null
                ...
                : Check("Backup", "Latest: " + report.LatestBackup.Manifest.CreatedAtUtc.ToLocalTime().ToString("g"), HealthSeverity.Healthy, "backups"));

            InspectManagedFiles(instance, report);
            var crashRoot = Path.Combine(instance.DataDir, "crash-reports");
            var latestCrash = Directory.Exists(crashRoot) ? Directory.GetFiles(crashRoot, "*.txt").OrderByDescending(File.GetLastWriteTimeUtc).FirstOrDefault() : null;
            if (latestCrash != null)
        ...
        private static void InspectManagedFiles(InstalledInstance instance, InstanceHealthReport report)
        {
            ...
            if (!File.Exists(path))
            {
                report.Checks.Add(Check("Managed files", "Local profile; no managed-file manifest is available.", HealthSeverity.Information));
                return;
            }
            try
            {
                ...
                var invalid = manifest.Files.Where(file =>
                {
                    ...
                    return !File.Exists(candidate) || new FileInfo(candidate).Length != file.Size ||
                           (!string.IsNullOrWhiteSpace(file.Sha256) && !HashService.Sha256(candidate).Equals(file.Sha256, StringComparison.OrdinalIgnoreCase));
                }).Take(10).ToList();
                report.Checks.Add(invalid.Count == 0
                    ? Check("Managed files", "All managed files are present.", HealthSeverity.Healthy)
                    : Check("Managed files", invalid.Count + " managed file(s) need repair.", HealthSeverity.Warning, "repair"));
            }
            catch
            {
                report.Checks.Add(Check("Managed files", "The managed-file manifest is invalid.", HealthSeverity.Warning, "repair"));
            }
        }

        private static HealthCheckItem Check(string name, string message, HealthSeverity severity, string action = null) =>
