...
namespace TCLauncher.Core.Services
{
    ...
    public sealed class CatalogService : ICatalogService
    {
        ...
        public CatalogService(HttpClient http, Uri v2Uri, Uri legacyUri, string cachePath, IAtomicFileService files,
            ILogService log)
        {
            _http = http;
            _v2Uri = v2Uri;
        ...
        public async Task<OperationResult<CatalogLoadResult>> LoadAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                using (var request = new HttpRequestMessage(HttpMethod.Get, _v2Uri))
                {
                    if (File.Exists(_etagPath))
                        request.Headers.TryAddWithoutValidation("If-None-Match", File.ReadAllText(_etagPath));
                               ...
                               cancellationToken))
                    {
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            if (response.Headers.ETag != null)
                                _files.WriteAllText(_etagPath, response.Headers.ETag.ToString());
                            return OperationResult<CatalogLoadResult>.Success(
                                new CatalogLoadResult { Catalog = catalog }, operationId);
                        }
                    }
                }
            ...
            {
                return OperationResult<CatalogLoadResult>.Failure(LauncherErrorCode.Cancelled,
                    "Catalog loading was cancelled.", exception, operationId);
            }
        ...
        private async Task<OperationResult<CatalogLoadResult>> LoadLegacyOrCache(string operationId,
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                return OperationResult<CatalogLoadResult>.Success(new CatalogLoadResult { Catalog = document },
                    operationId);
            }
            catch (Exception exception)
            {
                ...
                if (File.Exists(_cachePath))
                    return OperationResult<CatalogLoadResult>.Success(ReadCache(true), operationId);
                return OperationResult<CatalogLoadResult>.Failure(LauncherErrorCode.NetworkUnavailable,
                    "The catalog is unavailable and no cached copy exists.", exception, operationId);
            }
        }
        ...
        private CatalogLoadResult ReadCache(bool offline)
        {
            ...
            {
                Catalog = catalog, IsOffline = offline, IsStale = DateTime.UtcNow - cachedAt > TimeSpan.FromDays(2),
        ...
        {
            if (catalog == null || catalog.SchemaVersion != 2)
